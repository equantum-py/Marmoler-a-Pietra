            ...
            {surfaces.map((material) => (
              <article key={material.slug} className="product-card overflow-hidden rounded-md">
                ...
                <div className="p-3 text-center">
                  ...
                  <a href={buildWhatsappUrl(`Hola, quiero cotizar ${material.name}.`, settings.whatsapp_number)} target="_blank" rel="noreferrer" className="inline-flex min-h-8 w-full items-center justify-center rounded bg-pietra-green px-3 text-[11px] font-extrabold uppercase text-white transition hover:bg-pietra-sage">Cotizar</a>
                </div>
              </article>
            ))}
          </div>
          <a href={buildWhatsappUrl('Hola, quiero una cocina a medida con superficies premium.', settings.whatsapp_number)} target="_blank" rel="noreferrer" className="group relative min-h-[360px] overflow-hidden rounded-md border border-pietra-border bg-pietra-ink shadow-card">
